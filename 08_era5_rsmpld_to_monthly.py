...
# > 1. converts the daily resampled ERA5 data to netcdf   
...
# %% [markdown]
## Read files 
rasters = sorted(glob.glob("../../3_RSData/1_Rasters/4_ERA5/daily/03_TiffResampled/*.tif"))
...
# %% [markdown]
## Convert to netcdf 
outDir = "../../3_RSData/1_Rasters/4_ERA5/daily/04_TiffRsmpldToNC/"
fresh(outDir)
...
for i in td(range(len(rasters)), desc='Converting'):
    ...
    da = da.rename('ERA5')  # variable given name 
...
ds.to_netcdf(outDir + "SingleFile/" + 'era5.nc')
...
ds = xr.open_dataset(outDir + "SingleFile/" + 'era5.nc')
ds = ds.resample(time='MS').sum()  

# [2] save
outDir1 = "../../3_RSData/1_Rasters/4_ERA5/daily/05_NCToNCMonthly/"
fresh(outDir1)
