...
# > 1. converts the daily resampled APHRODITE data to netcdf   
...
# %% [markdown]
## Read files 
rasters = sorted(glob.glob("../../3_RSData/1_Rasters/6_APHRODITE/V1101/daily/02_TiffResampled/*.tif"))
...
# %% [markdown]
## Convert to netcdf 
outDir = "../../3_RSData/1_Rasters/6_APHRODITE/V1101/daily/03_RsmpldToNCDaily/"
fresh(outDir)
...
for i in td(range(len(rasters)), desc='Converting'):
    ...
    da = da.rename('APHRO')  # variable given name 
...
ds.to_netcdf(outDir + "SingleFile/" + 'aphro.nc')
...
ds = xr.open_dataset(outDir + "SingleFile/" + 'aphro.nc')
ds = ds.resample(time='MS').sum()  

# [2] save
outDir1 = "../../3_RSData/1_Rasters/6_APHRODITE/V1101/daily/04_NCDailyToMonthly/"
fresh(outDir1)
for i in td(range(len(ds.time)), desc='Saving'):
    date = str(ds.time[i].values).split('T')[0]  # date extracted
    outName = outDir1 + date + '.nc'
